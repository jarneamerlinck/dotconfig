...
# print("Error: ", "sensor_id", "ts_ids_key", "ts_ids_value")   

# ts_ids = ts_id_mapping["iot_waterquantity_geolux_smartdatalogger_0001"]
# # ts_ids_request = {f"id{j}" value for j, (key, value) in enumerate(ts_ids.items())}
# ts_ids_request = {}

# for j, (key, value) in enumerate(ts_ids.items()):
#     ts_ids_request[f"id{j}"] = value
#     r = requests.get(
#                 BASE_URL,
#                 params={f"id{j}":value, "start": d_start_unixtime, "end": d_end_unixtime},

#             )
#     print(pd.read_csv(io.StringIO(r.content.decode("utf-8"))))
...
# r = requests.get(
#                 BASE_URL,
#                 params={**ts_ids_request, "start": d_start_unixtime, "end": d_end_unixtime},
#             )
# data = pd.read_csv(io.StringIO(r.content.decode("utf-8")))
  
  
           
for sensor_id in ["iot_waterquantity_geolux_smartdatalogger_0001", "iot_waterquantity_geolux_smartdatalogger_0002", "iot_waterquantity_geolux_smartdatalogger_0003", "iot_waterquantity_geolux_smartdatalogger_0004"]:
    ts_ids = ts_id_mapping[sensor_id]
    ts_ids_request = {f"id{j}": value for j, (key, value) in enumerate(ts_ids.items())}
    for ts_ids_key, ts_ids_value in ts_ids_request.items():
        try:
            r = requests.get(
                BASE_URL,
                params={ts_ids_key: ts_ids_value, "start": d_start_unixtime, "end": d_end_unixtime},
            )
            data = pd.read_csv(io.StringIO(r.content.decode("utf-8")))
            #print(sensor_id, ts_ids_key, ts_ids_value) 
        except:
            # print("Error: ", sensor_id, ts_ids_key, ts_ids_value)     
            log.error("Error: ", sensor_id, ts_ids_key, ts_ids_value)     
